...
router.get('/templates/current', async (req, res) => {
  try {
    ...
    logger.error('Error getting current template:', error);
    res.status(500).json({
      success: false,
...
router.get('/templates/preview-data', (req, res) => {
  try {
    ...
    logger.error('Error generating data preview:', error);
    res.status(500).json({
      success: false,
...
router.post('/generate', ensureAuthenticated, async (req, res) => {
  try {
    ...
    const result = await noiService.generateNoiDocument(claimData);
 ...
 */
router.post('/generate-pdf', ensureAuthenticated, async (req, res) => {
  try {
    const claimData = req.body;
    ...
    // Generate the PDF
    const result = await noiService.generateNoiPdf(claimData);
...
router.post('/generate/:claimId', ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (!fs.existsSync(result.documentPath)) {
      logger.error('Generated file does not exist:', result.documentPath);
      return res.status(500).json({
        success: false,
    ...
    logger.error('NOI generation error:', error);
    return res.status(500).json({
      success: false,
